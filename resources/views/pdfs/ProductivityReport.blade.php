...
<head>
    ...
    <title>Reporte de productividad de {{$user}}</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    ...
    <div class="header">
        <div class="logo-wrapper">
            ...
            <h1>Reporte de productividad de {{$user}}</h1>
        </div>
        <h2>Actividades concluidas periodo {{$date1}} - {{$date2}} :</h2>
    </div>
    ...
    <table class="table">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>{{ \Carbon\Carbon::parse($card->updated_at)->format('d/m/Y') }}</td>
                <td>{{ $card->days_late <= 0 ? 0 : $card->days_late }}</td>
            </tr>
            @endforeach
        </tbody>
    </table>
 <!-- Espacio para la gráfica $lineChartBase64 -->
    ...
    <div class="chart-wrapper">
        <img src="data:image/png;base64,{{$lineChartBase64}}" alt="Gráfica de lineas">
    </div>
